...
def buildAPIUrl(KISMET_HTTP_PROTOCOL, KISMET_HOST, KISMET_PORT, MAC_ADDR, DEBUG, VERBOSE):
    if validateMac(MAC_ADDR):
        if DEBUG:
            print("%s://%s:%s/devices/by-mac/%s/devices.json" % (KISMET_HTTP_PROTOCOL, KISMET_HOST, KISMET_PORT, MAC_ADDR))
        if VERBOSE:
            print("- MAC address validated")
        api_url = ("%s://%s:%s/devices/by-mac/%s/devices.json" % (KISMET_HTTP_PROTOCOL, KISMET_HOST, KISMET_PORT, MAC_ADDR))
    else:
